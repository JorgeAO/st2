...
class CtrlPrestamos extends Control
{
	...
	public function intCalcular($arrParametros)
	{
		try 
		{
			...
			$flCuota = $flTotalPago / $arrParametros['pres_plazo'];
	...
	public function insertar($arrParametros)
	{
		try 
		{
			...
			for ($i = 1; $i <= $arrParametros['pres_plazo']; $i++)
			{
				ClsCuotas::insertar([
					'fk_pre_prestamos' => $arrPrestamo['insert_id'],
					'prcu_numero' => $i,
					'prcu_fecha' => date('Y-m-d', strtotime('+'.$i.' months', strtotime($arrParametros['pres_fecha']))),
